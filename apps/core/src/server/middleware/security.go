...
func Register(app *fiber.App, cfg *config.Config, log *zap.Logger) {
	...
	// 2. CORS: controla qué orígenes pueden llamar a la API.
	// AllowCredentials permite cookies/tokens de sesión desde el frontend.
	...
	app.Use(cors.New(cors.Config{
		...
		MaxAge:           3600, // caché del preflight: 1 hora
	}))

	// 3. Rate Limiter: máximo N peticiones por ventana de tiempo por IP.
	// Cuando se supera el límite responde 429 en vez de procesar la petición.
