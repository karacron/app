...
// ValidateStruct valida un struct contra sus tags de validación
// Retorna un map de errores por campo si hay validación fallida
...
func ValidateStruct(data interface{}) map[string]string {
	...
	if err := validatorInstance.Struct(data); err != nil {
		...
		for _, fieldError := range validationErrors {
			...
			tag := fieldError.Tag()
			errors[fieldName] = "Validación fallida: " + tag
		}
	}
...
func ValidatorMiddleware(dataType interface{}) fiber.Handler {
	return func(c fiber.Ctx) error {
		// El body ya será parseado por BodyParser en el handler anterior
		// Este middleware es principalmente para documentación/ejemplo
		return c.Next()
	}
}

// ValidateRequest es una función helper para validar request body dentro de un handler
//
// Uso en handler:
...
// Tags de validación disponibles (ejemplos):
// required     - campo obligatorio
// max=N        - máximo N caracteres/elementos
// min=N        - mínimo N caracteres/elementos
// email        - validar formato email
// url          - validar formato URL
// numeric      - solo números
// alpha        - solo letras
// alphanum     - letras y números
// gte=N        - mayor o igual a N (para números)
// lte=N        - menor o igual a N (para números)
// gt=N         - mayor que N
// lt=N         - menor que N
